...
export const CustomCard = <T,>({children, className, isActive , withHover, isLoading, withGlowEffect, withBlur, withOpacity}: Props) => {
  ...
  const blur = withBlur ? 'transition duration-300 filter blur-xs group-hover:blur-none' : ''
  const opacity = withOpacity ?  isHovered ? 'opacity-100' : `opacity-70` : `opacity-100` 
  return (
    <div className={`relative cursor-pointer group`  }
      onMouseEnter={() =>{ setIsHovered(true); setColors(activeColors)}}
      onMouseLeave={() =>{ setIsHovered(false); setColors(defaultColors)}}
    >
    <MagicCard className={`bg-card rounded-xl ${className} ${hover} ${opacity}`}>
      { isLoading && (
